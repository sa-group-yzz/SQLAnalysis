...
import top.viewv.analysis.helper.APIToQueryProcessor;
import top.viewv.analysis.helper.Parameter;
import top.viewv.analysis.helper.StringResult;
...
public class QueryAnalysis {
	...
	public QueryAnalysis(StringCallGraph callGraph)
	{
		...
		if(ANALYZE_RAW)
		{
			...
			// JDBC
			target.put("<java.sql.Statement: boolean execute(java.lang.String)>",paraSet);
			target.put("<java.sql.Statement: java.sql.ResultSet executeQuery(java.lang.String)>",paraSet);
		...
		{
			if(!sig.contains("execSQL") && !sig.contains("rawQuery") && !sig.contains("executeQuery") && !sig.contains("execute"))
				sqliteAPIs.add(sig);
		}
